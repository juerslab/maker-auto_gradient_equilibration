...
#		print(self.p_auto_units, self.p_auto_syringe_area, self.p_auto_speed, self.p_auto_accel)
	...
	def auto_run(self):
		...
		if (not active_and_different):
			...
			activation_instrucitons = [False,False,False]
			
			activation_instrucitons[int(self.p_auto_inject) - 1] = True
			activation_instrucitons[int(self.p_auto_extract) - 1] = True
			
			self.ui.p1_activate_CHECKBOX.setChecked(activation_instrucitons[0])
			self.ui.p2_activate_CHECKBOX.setChecked(activation_instrucitons[1])
			self.ui.p3_activate_CHECKBOX.setChecked(activation_instrucitons[2])
	...
	def auto_update(self, injection_n, volume, units, area, microstepping, inject, extract):
		...
		if self.is_premix_active:
			...
			p_extract_displacement = str(self.convert_displacement(2*volume, units, area, microstepping))
	
			pump_instruction_list[int(inject) - 1] = p_inject_displacement
			pump_instruction_list[int(extract) - 1] = p_extract_displacement

			cmd_forward1 = "<RUN,DIST," + pumps_2_run + ",0,F,%s,%s,%s>" % tuple(pump_instruction_list)

			testData.append(cmd_reverse)
			testData.append(cmd_forward1)
		else:
			p_inject_displacement = str(self.convert_displacement(-volume, units, area, microstepping))
			p_extract_displacement = str(self.convert_displacement(volume, units, area, microstepping))
			...
			cmd_forward0 = "<RUN,DIST," + pumps_2_run + ",0,F,%s,%s,%s>" % tuple(pump_instruction_list)

			testData.append(cmd_forward0)
...
#		testData.append(cmd)
	...
	def set_linear_total_time(self):
		if self.linear_m != 0 and (self.linear_target_concentration - self.linear_initial_concentration) > 0:
			self.experiment_time_length = (self.linear_target_concentration - self.linear_initial_concentration) / self.linear_m
		else:
			self.experiment_time_length = 0
		
		self.linear_total_time = self.experiment_time_length / self.linear_duty_ratio
	...
	def set_linear_concentration_slope(self):
		average_m = self.ui.linear_concentration_slope_INPUT.value()
		
		self.linear_m = average_m / self.linear_duty_ratio
#		print(self.linear_m)
		#print(self.linear_m)
	...
	def linear_display_initial_injection_size(self):
		...
		time_interval = self.experiment_time_length / self.auto_injection_number
	...
	def linear_run(self):
		...
		if (not active_and_different):
			...
			self.ui.p1_accel_INPUT.setValue(100 * activation_instrucitons[0])
			self.ui.p2_accel_INPUT.setValue(100 * activation_instrucitons[1])
			self.ui.p3_accel_INPUT.setValue(100 * activation_instrucitons[2])
			...
			self.experiment_time_length = (self.linear_target_concentration - self.linear_initial_concentration) / self.linear_m
			injection_rate = self.linear_total_time / self.auto_injection_number
			...
			inject = self.p_auto_inject
			extract = self.p_auto_extract
			total_time = self.linear_total_time
			exp_time = self.experiment_time_length
			...
			if self.linear_duty_ratio != 1:
				self.linear_timer.start(injection_rate * 1000)
	...
	def linear_update(self, injection_n, cont_volume, units, area, microstepping, inject, extract, exp_time, ini_cons, syr_cons, m):
		...
		if self.is_premix_active:
			...
			p_extract_displacement = str(self.convert_displacement(2*inject_volume, units, area, microstepping))
		...
		#print(cmd)
		#testData.extend([inject_speed, extract_speed, cmd])
		thread = Thread(self.runTest, testData)
		thread.finished.connect(lambda:self.thread_finished(thread))
		thread.start()
		time.sleep(0.1)
...
def main():
	...
	window = MainWindow()
	window.setWindowTitle("Poseidon Pumps Controller - Pachter Lab Caltech 2018 - Modified by Juers Lab Whitman College 2022")
	window.show()
