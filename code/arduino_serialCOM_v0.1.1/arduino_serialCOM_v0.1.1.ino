...
void setup_microstepping() {

  // Set mode pins as outputs
  ...
  // Set logic levels for 1/16-step mode
...
void sendDistanceToPC(int mID,signed long dtogo) {

	if (dtogo != oldDistanceLeft[mID]){
		Serial.print("<DISP|");
		Serial.print(mID);
		Serial.print("|");
        Serial.print(dtogo);
...
void runFew() {
  ...
  for (int i = 0; i < 3; i += 1) {
    if (motors[i] == 1) {
      toMove[i] = direction * distances[i];
      Serial.print("Sending tomove command: ");
      Serial.println(toMove[i]);
      steppers[i].move(toMove[i]);
    }
  }
